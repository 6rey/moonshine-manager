...
@app.delete("/vm/delete/{vm_id}", tags=["VM"])
def delete_vm(
    vm_id: int,
    db: Session = Depends(get_db),
    ...
    Only admin or master can delete VMs.
    This will also delete all assignments related to this VM.
    """
    # Check if user is admin or master
    if current_user.role not in ["admin", "master"]:
        raise HTTPException(
            status_code=403,
            detail="Only admin or master can delete VMs"
        )
    ...
    # Delete all assignments for this VM first
    db.query(Assignment).filter(Assignment.vm_id == vm_id).delete()
    
    # Delete the VM
    db.delete(vm)
    db.commit()
    
    return {"msg": f"VM '{vm_hostname}' (ID: {vm_id}) deleted successfully"}
