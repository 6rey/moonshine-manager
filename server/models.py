...
class User(Base):
    ...
    email = Column(String(255), unique=True, nullable=True)  # NEW: For SSO
    sso_linked = Column(String(20), nullable=True)  # NEW: 'google', 'microsoft', etc.
...
# NEW: SSO Account linking table
class SSOAccount(Base):
    """Store SSO provider links for users"""
    ...
    # Unique constraint: one SSO account per provider per user
    __table_args__ = (
        UniqueConstraint('provider', 'provider_user_id', name='uix_sso_provider_user'),
    )
